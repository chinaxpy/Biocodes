...
head(gt)
head(ad)
mask0 <- which( gt[,"pa1K"] == "1/1"  &  gt[,"pa2G"] == "0/0" &  ( gt[,"po6K"] == "1/1"  |  gt[,"po6K"] == "0/1" ) ) 

ad_flt0 <- as.matrix(ad[mask0,c("po6K")])
colnames(ad_flt0) <- c("S_Pool")


ED_list0 <- apply(ad_flt0, 1, function(x){
  count <- as.numeric(unlist(strsplit(x, ",",fixed = TRUE,useBytes = TRUE)))
  depth <- count[1] + count[2]
  ED <- count[2] / depth
  return(ED)
})

mask1 <- which( gt[,"pa1K"] == "0/0"  &  gt[,"pa2G"] == "1/1" &  ( gt[,"po6K"] == "0/0"  |  gt[,"po6K"] == "0/1" ) ) 

 
ad_flt1 <- as.matrix(ad[mask1,c("po6K")])
colnames(ad_flt1) <- c("S_Pool")
